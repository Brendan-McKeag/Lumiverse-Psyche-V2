...
export function updateSystemPrompt(directive: string, includeRubrics = true): string {
  return [
    ...
    '  • Introduce supporting characters the story brings in (create_character) and',
    '    set who is present (set_present).',
    '  • Occasionally nudge a baseline (set_baseline) when a lasting change of',
    '    temperament is earned — not every turn.',
    '',
    'ECONOMY. Make the changes THIS turn warrants, then stop. When done, reply with a',
...
export function updateUserContent(run: RunState, transcript: string, cardContext: string): string {
  return [
    ...
    'happened (apply_stimulus, occasionally set_emotion/set_baseline), adjust',
    'approval, and add/admit any new characters. Be economical.',
  ]
    .filter(Boolean)
    .join('\n')
}
