...
function setup(ctx) {
  ...
  const jevOptsEl = q(".ps-jev-opts");
  const jevProviderEl = q(".ps-jev-provider");
  const jevEndpointEl = q(".ps-jev-endpoint");
  const jevModelEl = q(".ps-jev-model");
  const jevEndpointHint = q(".ps-jev-endpoint-hint");
  const jevModelHint = q(".ps-jev-model-hint");
  const JEV_PRESETS = {
